...
static PyObject *infer_ancestral_states(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "ss|sssssssi", &annotation_name, &tree_name, &out_annotation_name,
                          &out_tree_name, &prob_method, &out_param_name, &model, &in_param_name, &out_mp_name, &quiet)) {
...
static PyMethodDef PastmlMethods[] =
        {
                {"infer_ancestral_states", infer_ancestral_states, METH_VARARGS,
                        ...
                        "                             can be one of the max likelihood (ML) methods: pastml.MARGINAL_APPROXIMATION (default, marginal posterior probabilities approximation (MPPA)),\n"
                        "                             pastml.MARGINAL, pastml.MAX_POSTERIORI (maximum a posteriori MAP), pastml.JOINT;\n"
                        "                             or one of the max parsimony (MP) methods: pastml.DOWNPASS, pastml.DELTRAN (delayed transformation),\n"
                        "                             or pastml.ACCTRAN (accelerated transformation)\n"
                        "   :param out_param_file: str, if specified, the model parameters (state frequencies and scaling factor for ML, or number of character state changes for MP)\n"
