...
void write_subtree_to_stream(Node* node, Node* node_from, FILE* stream, double epsilon, double scaling) {
	...
	if(n == 1) {
		/* terminal node */
                if(node->br[0]->brlen == 0.0) {
                  fprintf(stream, "%s:%f", (node->name ? node->name : ""), node->br[0]->brlen + epsilon); /* distance to father */
                } else {
		  fprintf(stream, "%s:%f", (node->name ? node->name : ""), node->br[0]->brlen * scaling); /* distance to father */
                }
	} else {
		...
		for(i=1; i < n-1; i++) {
			write_subtree_to_stream(node->neigh[(direction_to_exclude+i) % n], node, stream, epsilon, scaling); /* a son */
			putc(',', stream);
		}
		write_subtree_to_stream(node->neigh[(direction_to_exclude+i) % n], node, stream, epsilon, scaling); /* last son */
		putc(')', stream);
                if(node->br[0]->brlen == 0.0) {
                  fprintf(stream, "%s:%f", (node->name ? node->name : ""), node->br[0]->brlen + epsilon); /* distance to father */
                } else {
...
void write_nh_tree(Tree* tree, FILE* stream, double epsilon, double scaling) {
	...
	putc('(', stream);
	for(i=0; i < n-1; i++) {
		write_subtree_to_stream(node->neigh[i], node, stream, epsilon, scaling); /* a son */
		putc(',', stream);
	}
	write_subtree_to_stream(node->neigh[i], node, stream, epsilon, scaling); /* last son */
	putc(')', stream);
