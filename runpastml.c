...
int calculate_frequencies(size_t num_annotations, size_t num_tips, int *states, char **character, char *model,
                          double *parameters, double scaling_factor, double epsilon) {
    ...
    if(strcmp(model, HKY)==0){
        setHKYFrequencies(parameters);
        parameters[num_annotations] = 1.0;
        parameters[num_annotations + 1] = 0.0;
    } else if(strcmp(model,JTT)==0){
        setJTTFrequencies(parameters, num_annotations);
        parameters[num_annotations] = 1.0;
        parameters[num_annotations + 1] = 0.0;
    } else {
        parameters[num_annotations] = scaling_factor;
        parameters[num_annotations + 1] = epsilon;

        count_array = calloc(num_annotations + 1, sizeof(int));
        if (count_array == NULL) {
...
int is_valid_model(char* model) {
    return (strcmp(model, JC) == 0) || (strcmp(model, F81) == 0)
           || (strcmp(model, HKY) == 0) || (strcmp(model, JTT) == 0);
}
...
int runpastml(char *annotation_name, char *tree_name, char *out_annotation_name, char *out_tree_name,
    ...
    if (is_parsimonious) {
        ...
        exit_val = calculate_frequencies(num_annotations, num_tips, states, character, model, parameters,
                                         1.0 / s_tree->avg_branch_len,
                                         MIN(s_tree->min_branch_len, s_tree->avg_tip_branch_len / 10.0));
