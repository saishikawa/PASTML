...
int main(int argc, char **argv) {
    ...
    const char *help_string = "usage: PASTML -a ANNOTATION_FILE -t TREE_NWK [-o OUTPUT_ANNOTATION_CSV] [-n OUTPUT_TREE_NWK] [-q] "
            ...
            "                                       can be one of the max likelihood (ML) methods: marginal_approx (default, marginal posterior probabilities approximation (MPPA)),\n"
            "                                       marginal, max_posteriori (maximum a posteriori MAP), joint;\n"
            "                                       or one of the max parsimony (MP) methods: downpass, acctran (accelerated transformation), or deltran (delayed transformation)\n"
            "   -r OUTPUT_PARAMETERS_CSV            if specified, the model parameters (state frequencies and scaling factor for ML, or number of character state changes for MP)\n"
