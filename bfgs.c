...
void gradient(Node *nd, char** tipnames, int* states, int nb, int nbanno, double mu, char* model, double* p, double* g, int n) {
  ...
  for(i=0;i<n;i++){
    ...
    if(i<nbanno) {
      g[i] = (fabs(pluslnl) - fabs(oldlnl)) / (STEP*100);
    } else if (i==nbanno) {
...
void lnsrch(Node *nd, char** tipnames, int* states, int nb, int nbanno, double mu, char* model, int n, double* xold, double fold, double* g, double* p, double* x, double *f, double stpmax, int *check) {
...
void dfpmin(Node *nd, char** tipnames, int* states, int nb, int nbanno, double mu, char* model, double *p, int n, int *iter, double *fret) {
...
function). The routine lnsrch is called to perform approximate line minimizations.*/
  ...
  for(its=0;its<ITMAX;its++) {
    *iter=its; 
    ...
    lnsrch(root, tipnames, states, nb, nbanno, mu, model, n, p, fp, g, xi, pnew, fret, stpmax, &check);
    ...
    for(i=0;i<n;i++){
      ...
      if(i==nbanno+1) {
        if(pnew[i] < DBL_MIN*1.0e+4) pnew[i] = pold[i];
        if(pnew[i] > s_tree->min_bl*1.0e+4) pnew[i] = pold[i];
      }
      p[i]=pnew[i];
    }
    test=0.0;
